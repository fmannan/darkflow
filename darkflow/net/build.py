...
import json
import os
from axnet.isp.tf_isp_core import ISP
...
class TFNet(object):
    ...
    def __init__(self, FLAGS, darknet=None):
        ...
        self.isp = None
        if self.FLAGS.isp:
            isp_spec = self.FLAGS.isp
            print('isp_spec: ', isp_spec)
            if '.json' not in isp_spec and '.npz' not in isp_spec:
                print(isp_spec)
                isp_spec = json.loads(isp_spec)

            self.isp = ISP(isp_spec=isp_spec)
    ...
    def build_forward(self):
        ...
        if self.isp is not None:
            isp_out = self.isp.build(self.inp, is_training=self.train)
            state = identity(isp_out)
        else:
